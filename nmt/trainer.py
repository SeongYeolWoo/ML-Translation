...
from nmt.utils import get_parameter_norm, get_grad_norm
...
class MaximumLikelihoodEstimationEngine(Engine):
    ...
    def train(engine, mini_batch):
        ...
        mini_batch.tgt = (mini_batch.tgt[0].to(device), mini_batch.src[1])

        x, y = mini_batch.src, mini_batch.tgt[0][:, 1:]
        # mini_batch.tgt[0] = '<BOS>'
        # |x| = (batch_size, length)
        # |y| = (batch_size, length)
        with autocast():
    ...
    @staticmethod
    def attach(train_engine, valid_engine, train_metric_names = ['loss', 'ppl', '|param|', '|g_param|'], valid_metric_names = ['loss', 'ppl'], verbose = VERBOSE_BATCH_WISE):
        ...
        if verbose >= VERBOSE_EPOCH_WISE:
            ...
            def print_valid_logs(engine):
                ...
                print('Validation - loss={:.4e}  ppl={:.2f}  best_loss={:.4e}  best_ppl={:.2e}'.format(
                    avg_loss,
                    np.exp(avg_loss),
                    engine.best_loss,
...
class SingleTrainer():
    ...
    def train(self, model, crit, optimizer, train_loader, valid_loader, src_vocab, tgt_vocab, n_epochs, lr_scheduler):
        train_engine = self.target_engine_class(
            self.target_engine_class.train,model,crit, optimizer, lr_scheduler, self.config
        )
        valid_engine = self.target_engine_class(
            self.target_engine_class.validation, model, crit, optimizer = None, lr_scheduler = None, config = self.config
        )

        self.target_engine_class.attach(train_engine, valid_engine, verbose = self.config.verbose)
        ...
        valid_engine.add_event_handler(
            ...
            train_engine,
            self.config, src_vocab, tgt_vocab,
        )
