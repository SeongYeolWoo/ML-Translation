...
def overwrite_config(config, prev_config):
    ...
    for prev_key in vars(prev_config).keys():
        if not prev_key in vars(config).keys():
            print('WARNING!!!! Argument "--%s" is not found in current argument parser.\tIgnore saved value: '% prev_key, vars(prev_config)[prev_key])

    for key in vars[config].keys():
        if not key in vars(prev_config).keys():
            print('WARNING!!!! Argument "--%s" is not found in saved  model.\tUse current value: '%key, vars(config)[key])

        elif vars(config)[key] != vars(prev_config)[key]:
            print('WARNING!!!! You changed value for argument "--%s".\tUse current value: '%key, vars(config)[key])
